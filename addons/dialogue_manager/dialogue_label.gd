...
func _type_next(delta: float, seconds_needed: float) -> void:
	...
	if _last_wait_index != visible_characters and additional_waiting_seconds > 0:
		...
		var prev_line = get_character_line(visible_characters - 1)
		visible_characters += 1
		var new_line = get_character_line(visible_characters - 1)
		if prev_line != new_line:
			var line_height = get_content_height() / get_line_count()
			var displayable_lines_count = int(size.y / line_height)
			var visible_lines_count = get_visible_line_count()
			if new_line >= displayable_lines_count:
				var v_scroll_bar = get_v_scroll_bar()
				var scroll_value = v_scroll_bar.value
				scroll_to_line(new_line - displayable_lines_count + 1)
				var new_value = v_scroll_bar.value
				v_scroll_bar.set_value_no_signal(scroll_value)
				#create_tween().tween_method(v_scroll_bar.set_value_no_signal, scroll_value, new_value, 0.2)
				create_tween().tween_property(v_scroll_bar, "value", new_value, 0.2)
